...
class BaselineJavaVersionIntegrationTest {
    ...
    @Test
    void java_11_compilation_fails_targeting_java_8(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 8
                runtime = 11
        ...
        project.mainSourceSet().java().writeClass(JAVA_11_COMPATIBLE_CODE);

        gradle.withArgs("compileJava").buildsWithFailure();
    }

    @Test
    void distribution_target_is_used_when_no_artifacts_are_published(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
        ...
        File compiledClass =
                project.path().resolve("build/classes/java/main/Main.class").toFile();
        assertBytecodeVersion(compiledClass, JAVA_17_BYTECODE, NOT_ENABLE_PREVIEW_BYTECODE);
    }

    @Test
    void java_17_preview_compilation_works(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
        ...
        project.mainSourceSet().java().writeClass(JAVA_17_PREVIEW_CODE);
    ...
    @Test
    void setting_library_target_to_preview_version_fails(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
    ...
    @Test
    void java_17_preview_on_single_project_works(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersion {
                runtime = '17_PREVIEW'
                target = '17_PREVIEW'
    ...
    @Test
    void java_17_preview_javadoc_works(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
                distributionTarget = '17_PREVIEW'
    ...
    @Test
    void distribution_target_is_used_when_sls_packaging_is_used(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
    ...
    @Test
    void java_11_compilation_succeeds_targeting_java_11(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = '11'
            }
    ...
    @Test
    void java_11_execution_succeeds_on_java_11(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
            }
    ...
    @Test
    void java_11_execution_succeeds_on_java_17(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
                runtime = 17
    ...
    @Test
    void when_setup_jdk_toolchains_true_toolchains_are_configured_by_jdks_latest(
            GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
    ...
    @Test
    void when_setup_jdk_toolchains_false_no_toolchains_are_configured_by_gradle_baseline(
            GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
    ...
    @Test
    void java_8_execution_succeeds_on_java_8(GradleInvoker gradle, RootProject project) {
        org.assertj.core.api.Assumptions.assumeThat(System.getProperty("os.arch"))
                .describedAs(
        ...
        project.mainSourceSet().java().writeClass(JAVA_8_COMPATIBLE_CODE);
    ...
    @Test
    void java_8_execution_succeeds_on_java_11(GradleInvoker gradle, RootProject project) {
        org.assertj.core.api.Assumptions.assumeThat(System.getProperty("os.arch"))
                .describedAs(
                        "On an M1 mac, this test will fail to download"
                            + " https://api.adoptopenjdk.net/v3/binary/latest/8/ga/mac/aarch64/jdk/hotspot/normal/adoptopenjdk")
    ...
    @Test
    void java_plugin_convention_get_target_compatibility_produces_the_runtime_java_version(
            GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
    ...
    @Test
    void verification_should_fail_when_target_exceeds_the_runtime_version(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 17
                runtime = 11
    ...
    @Test
    void verification_should_succeed_when_target_and_runtime_versions_match(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 17
                runtime = 17
    ...
    @Test
    void can_configure_a_jdk_path_to_be_used(GradleInvoker gradle, RootProject project) throws IOException {
        org.assertj.core.api.Assumptions.assumeThat(System.getenv("CI"))
                .describedAs("This test deletes a directory locally, you don't want to run it on your mac")
                .isNotNull();

        Path newJavaHome =
                Files.createSymbolicLink(project.path().resolve("jdk"), Paths.get(System.getProperty("java.home")));
    ...
    @Test
    void check_runtime_classpath_compatible_fails_when_there_is_a_17_jar_on_the_runtime_classpath_but_runtime_is_11(
            GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
    ...
    @Test
    void
            check_runtime_classpath_compatible_succeeds_when_there_is_only_jars_of_the_compatible_java_runtime_versions_on_the_runtime_classpath(
                    GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 8
    ...
    @Test
    void check_runtime_classpath_compatible_handles_gradle_api(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 8
                runtime = 8
    ...
    @Test
    void check_runtime_classpath_compatible_is_a_dependency_of_check(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
                runtime = 11
    ...
    @Test
    void explain_java_versions_prints_the_java_version_used(GradleInvoker gradle, RootProject project) {
        project.buildGradle().append("""
            javaVersions {
                libraryTarget = 11
                runtime = 17
