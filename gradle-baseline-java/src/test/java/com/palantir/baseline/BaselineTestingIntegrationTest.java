...
class BaselineTestingIntegrationTest {
    ...
    void runs_Nebula_tests(GradleInvoker gradle, RootProject project) {
        ...
        project.file("src/test/groovy/test/Test.groovy")
                .overwrite("""
                        package test
    ...
    @Test
    void checkJUnitDependencies_run_as_part_of_check(GradleInvoker gradle, RootProject project) {
        standardBuildFile(project);
    ...
    @Test
    void running_recreate_true_will_re_run_tests_even_if_no_code_changes(
    ...
    @Test
    void does_not_crash_with_non_utf8_resources(GradleInvoker gradle, RootProject project) throws Exception {
        standardBuildFile(project);

        // Create a binary file that would crash during UTF-8 parsing
        project.file("src/test/resources/some-binary").createEmpty();
        java.nio.file.Files.write(
                project.path().resolve("src/test/resources/some-binary"),
