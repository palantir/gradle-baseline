...
class BaselineTestingIntegrationTest {
    ...
    void runs_test_sets_tests(GradleInvoker gradle, RootProject rootProject) {
        ...
        rootProject.file("src/integrationTest/java/test/JUnit5Test.java").overwrite("""
            package test;
    ...
    @Test
    void does_not_crash_with_non_utf8_resources(GradleInvoker gradle, RootProject rootProject) throws IOException {
        applyStandardBuildFile(rootProject);

        Path resourceFile = rootProject.path().resolve("src/test/resources/some-binary");
        Files.createDirectories(resourceFile.getParent());
        try (OutputStream out = Files.newOutputStream(resourceFile)) {
            // Invalid unicode sequence identifier
            out.write(new byte[] {(byte) 0xA0, (byte) 0xA1});
        }
