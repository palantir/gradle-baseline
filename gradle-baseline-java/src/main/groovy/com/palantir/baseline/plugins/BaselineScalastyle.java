    ...
    public void apply(Project project) {
        ...
        project.getPluginManager().withPlugin("scala", plugin -> {
            project.getPluginManager()
                    .withPlugin("nebula.dependency-recommender",
                            nebulaPlugin -> project.getExtensions()
                                    .configure(RecommendationProviderContainer.class,
                                            recommendations -> recommendations.excludeConfigurations("zinc")));
            JavaPluginConvention javaConvention = project.getConvention().getPlugin(JavaPluginConvention.class);
            project.getTasks()
                    .withType(ScalaCompile.class)
                    .configureEach(scalaCompile -> scalaCompile.getScalaCompileOptions()
                            .setAdditionalParameters(ImmutableList.of(
            ...
            scalaStyleTasks.configureEach(scalaStyleTask -> {
                ...
                javaConvention.getSourceSets()
                        .forEach(sourceSet -> sourceSet.getAllSource()
                                .getSrcDirs()
        ...
        ideaModel.getProject()
                ...
                .withXml(xmlProvider -> {
                    ...
                    Node scalaCompilerConf = (Node) rootNode
                            ...
                            .stream()
                            .filter(o -> ((Node) o).attributes()
                                    .get("name")
                                    .equals("ScalaCompilerConfiguration"))
                            .findFirst()
                            .orElseGet(() -> rootNode.appendNode(
                                    "component",
                                    ImmutableMap.of("name", "ScalaCompilerConfiguration")));
                    ...
                    Node parametersNode = (Node) scalaCompilerConf.getAt(new QName("parameters"))
                            .stream()
                            .findFirst()
                            .orElseGet(() -> scalaCompilerConf.appendNode("parameters"));
                    Node parameter = (Node) parametersNode.getAt(new QName("parameter"))
                            .stream()
