...
public class CheckBomConflictTask extends DefaultTask {
    ...
    public final void checkBomConflict() {
        ...
        Map<String, String> resolvedConflicts = parsedVersionsProps.forces().stream()
                ...
                .collect(Collectors.toMap(Pair::getLeft, Pair::getRight, BinaryOperator.maxBy(
                        BaselineVersions.VERSIONS_PROPS_ENTRY_SPECIFIC_COMPARATOR)));
        ...
        if (shouldFix.get()) {
            ...
            getProject().getLogger().lifecycle("Removing critical conflicts from versions.props:\n"
                    + toRemove.stream().map(name -> String.format(" - '%s'", name)).collect(Collectors.joining("\n")));
