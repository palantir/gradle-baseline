...
public final class PreferAssertj extends BugChecker implements BugChecker.MethodInvocationTreeMatcher {
    ...
    private static final Matcher<ExpressionTree> FAIL =
            MethodMatchers.staticMethod().onClassAny(LEGACY_ASSERT_CLASSES).named("fail").withParameters();
    ...
    public Description matchMethodInvocation(MethodInvocationTree tree, VisitorState state) {
        ...
        if (ASSERT_SAME_DESCRIPTION.matches(tree, state)) {
            return withAssertThat(tree, state, 2, (assertThat, fix) -> fix.replace(tree, assertThat
                    + ".describedAs("
        ...
        if (ASSERT_EQUALS_FLOATING.matches(tree, state)) {
            return withAssertThat(tree, state, 1, (assertThat, fix) ->
                    fix.addStaticImport("org.assertj.core.api.Assertions.within").replace(tree, String.format(
                            "%s%s",
                            assertThat,
        ...
        if (ASSERT_THAT.matches(tree, state)) {
            Optional<String> replacement = tree.getArguments().get(1).accept(HamcrestVisitor.INSTANCE, state);
            return withAssertThat(tree, state, 0, (assertThat, fix) -> fix.replace(tree, assertThat
        ...
        if (ASSERT_THAT_DESCRIPTION.matches(tree, state)) {
            Optional<String> replacement = tree.getArguments().get(2).accept(HamcrestVisitor.INSTANCE, state);
            return withAssertThat(tree, state, 1, (assertThat, fix) -> fix.replace(tree, assertThat
                    + ".describedAs("
                    + argSource(tree, state, 0)
                    + ")"
                    + replacement.orElseGet(() -> ".is(new "
    ...
    private Description withAssertThat(
        ...
        if (isIterableMap(actualArgument, state)) {
            actualArgumentString = String.format(
                    "(%s<?, ?>) %s", SuggestedFixes.qualifyType(state, fix, "java.util.Map"), actualArgumentString);
        }
    ...
    private static final class HamcrestVisitor extends SimpleTreeVisitor<Optional<String>, VisitorState> {
        ...
        private static final Matcher<ExpressionTree> NULL = Matchers.anyOf(
                MethodMatchers.staticMethod().onClass(MATCHERS).named("nullValue").withParameters(),
                MethodMatchers.staticMethod().onClass(MATCHERS).named("nullValue").withParameters("java.lang.Class"));

        private static final Matcher<ExpressionTree> NOT_NULL = Matchers.anyOf(
                MethodMatchers.staticMethod().onClass(MATCHERS).named("notNullValue").withParameters(),
                MethodMatchers.staticMethod()
                        .onClass(MATCHERS)
                        .named("notNullValue")
                        .withParameters("java.lang.Class"));
        ...
        public Optional<String> visitMethodInvocation(MethodInvocationTree node, VisitorState state) {
            ...
            if (HAS_ITEMS.matches(node, state) && isObjectVarArgs(node, state)) {
                ...
                return Optional.of(".contains("
                        + node.getArguments().stream().map(state::getSourceForNode).collect(Collectors.joining(", "))
                        + ')');
            }
